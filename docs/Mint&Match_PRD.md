...
**LLM provider:** Groq (`llama-3.3-70b-versatile`) — used ONLY for exception-reason generation, not for matching decisions.
...
Every Tier 3 case is passed to the **Exception Reasoner** (Groq `llama-3.3-70b-versatile`, plain chat completion, NOT a tool call) along with the unresolved record and its candidate list (possibly empty). The reasoner returns one specific, honest sentence explaining why resolution failed — referencing actual amounts/dates/candidate counts, and explicitly noting the "bank has no time data" structural limitation ONLY when it is actually the cause of the ambiguity.
